...
export class TaskTrackerController {
  ...
  @Get('user/:userId')
  async getTaskTrackersByUserId(
    @Param('userId') userId: string,
  ): Promise<TaskTracker[]> {
    const taskTrackers =
      await this.taskTrackerService.getTaskTrackersByUserId(userId);
    if (!taskTrackers || taskTrackers.length === 0) {
      throw new NotFoundException(
        `No TaskTrackers found for user with id ${userId}`,
      );
    }
    return taskTrackers;
  }
  ...
  async createTaskTrackerEntry(
    ...
    return this.taskTrackerService.createTaskTrackerEntry(
      title,
      storyPoints,
      status,
      comments,
